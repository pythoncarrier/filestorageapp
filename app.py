...
UPLOAD_FOLDER = r'E:\\+python_home\\projects\\filestorageapp\\files\\'


def show_files(username):
    return (os.listdir(path=UPLOAD_FOLDER+username))
...
def validate():
    ...
    if user!="" and password!="" and repeat_password!="":
        ...
        elif user not in get_user() and (password!=repeat_password or len(password)<6):
...
def delete():
    if request.method=='GET':
        ...
        print(choose)
        os.remove(path=UPLOAD_FOLDER+session.get('username', None)+'\\'+(choose))
        return redirect("http://127.0.0.1:4000/home",code=302)
